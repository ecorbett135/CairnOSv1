...
| Long Trail approach geometry source mapping | `trails/vermont_long_trail/raw/csv/approach_geometry_sources.csv` | Cairn project-authored source-to-approach mapping | Project-authored metadata; referenced geometry retains its own obligations | Stable IDs and exact feature metadata from `raw/geojson/gaia_reference.geojson` | Maps promoted `approach_id` / `geometry_id` values to exact source path, feature ID/title, source kind, source-license status, and geometry-only transformation notes | Eric Corbett / Cairn maintainers | 2026-07-18 | Mapping metadata does not grant rights to the referenced geometry; preserve `UNKNOWN — needs review` until source rights are confirmed. |
| Long Trail resupply amenities | `trails/vermont_long_trail/raw/csv/resupply_amenities.csv` | Long Trail Planning Guide plus manual coordinate/access-friction curation — needs review | UNKNOWN — needs review | `https://www.longtrailvermont.com/trail-town-amenities/` and manual map review — needs review | Curated town access, services, zero-candidate flags, structured access-friction fields, and road/trailhead coordinates | Eric Corbett / Cairn maintainers — needs review | 2026-05-19 | Do not assume source website content, access-friction values, or coordinates are Apache licensed. |
| Long Trail overnight amenities | `trails/vermont_long_trail/raw/csv/overnight_amenities.csv` | Green Mountain Club bear-box location page plus manual curation | UNKNOWN — needs review | `https://www.greenmountainclub.org/bear-boxes/` | Curated overnight-site bear-box availability by site name | Eric Corbett / Cairn maintainers — needs review | 2026-05-19 | Amenity enrichment only; verify current official source before field use. |
| Long Trail town service options | `trails/vermont_long_trail/raw/csv/town_service_options.csv` | Manual Cairn curation from candidate planning-guide research and independent business/official validation | UNKNOWN — needs review | Long Trail Planning Guide lodging/outfitter page plus listed validation URLs | Validated named lodging, food, outfitter, and mail-drop context for resupply town detail output | Eric Corbett / Cairn maintainers — needs review | 2026-05-19 | Advisory town-service enrichment only; do not bulk-copy third-party business lists and revalidate before field use. |
| Long Trail town lodging options | `trails/vermont_long_trail/raw/csv/town_lodging_options.csv` | Manual Cairn curation from maintainer lodging research and independent business/official validation URLs | UNKNOWN — needs review | User-provided `long_trail_vermont_lodging_research_20260520.csv` plus listed validation URLs | Curated active lodging candidates used to improve recovery confidence and town detail output; expanded with high-confidence southern approach and major recovery-town rows | Eric Corbett / Cairn maintainers — needs review | 2026-05-20 | Advisory lodging support only; does not include shuttle/transportation precision and must be revalidated before field use. |
| Long Trail side-trip options | `trails/vermont_long_trail/raw/csv/side_trip_options.csv` | Manual Cairn curation from candidate side-trip research and independent official/business validation | UNKNOWN — needs review | `https://www.longtrailvermont.com/side-trips/` plus listed validation URLs | Validated side-trip options for annotation-only user preferences | Eric Corbett / Cairn maintainers — needs review | 2026-05-19 | Experience annotation only; selected side trips do not alter itinerary time, mileage, feasibility, or export behavior. |
...
| Gaia reference waypoints and routes | `trails/vermont_long_trail/raw/geojson/gaia_reference.geojson` | Maintainer-owned Gaia GPS export / manual Gaia waypoints and routes — needs review | UNKNOWN — needs review | User-created Gaia data and possible map-derived geometry/waypoints — needs review | Raw reference enrichment; North Adams geometry source is feature `399a680d-2d50-440e-a22a-c82fd457f3fd`, title `North Adams Approach Trail`, updated `2026-04-18T19:12:44Z` | Eric Corbett / Cairn maintainers — needs review | 2026-04-18 | Do not redistribute as Apache without confirming waypoint/route source rights; source notes and personal metadata are not copied into the compiled approach artifact. |
...
| Compiled approach trails | `trails/vermont_long_trail/compiled/approach_trails.json` | Generated by Cairn compiler — needs review | Inherits input dataset obligations — needs review | `raw/csv/approach_trails.csv`, `raw/csv/approach_geometry_sources.csv`, and selected geometry-only features from `raw/geojson/gaia_reference.geojson` | Compiler output with stable approach/geometry IDs, mileage/terminus semantics, geometry coordinates, and exact minimal provenance; excludes raw source notes and non-geometry personal metadata | Cairn compiler | 2026-07-18 | Derived data; not automatically Apache licensed. North Adams is the only currently promoted branch geometry. |
...
As of 2026-07-18, the Vermont Long Trail runtime dataset has no validator
errors. Known warnings remain for:

- terrain sample miles using a different domain than guidebook overlay miles
- the existing operational graph's approach transitions lacking explicit
  endpoint locations until that derived artifact is next intentionally rebuilt
- incomplete grouped-town access-note coverage in some resupply rows
